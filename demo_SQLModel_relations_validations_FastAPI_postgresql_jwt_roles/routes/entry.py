...
def create(entry: EntryCreate, session: Session = Depends(get_session), current_user: dict = Depends(require_role("admin"))):
    try:
...
def read(entry_id: int, session: Session = Depends(get_session), current_user: dict = Depends(get_current_user)):
    try:
...
def read_by_author_name(author_name: str, session: Session = Depends(get_session), current_user: dict = Depends(get_current_user)):
    try:
...
def update_by_title(
    ...
    entry_data: dict = Body(
        ...
        examples={
            "example": {
                "summary": "Update Entry by Title Example",
                "value": {
                    "title": "Updated Entry Title",
                    "content": "Updated content for the entry"
                }
...
def delete(entry_id: int, session: Session = Depends(get_session), current_user: dict = Depends(require_role("admin"))):
    try:
