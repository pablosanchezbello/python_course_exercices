...
from crud.todo_list import create_todo_list, delete_todo_list, get_todo_list_filtered, update_todo_list

router = APIRouter()

@router.post("/", response_model=TodoList, status_code=201)
def create(todo_list: TodoListBase, session: Session = Depends(get_session)):
    """
    Create a new list.
    """
    try:
