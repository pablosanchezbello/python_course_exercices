...
def update_order_item(session: Session, order_item: OrderItem):
    ...
    if not existing_order_item:
        ...
        return delete_order_item_by_id(session, existing_order_item.order_id, existing_order_item.product_id)
    else:
...
def delete_order_item_by_id(session: Session, order_id: int, product_id: int):
    order_item = session.exec(select(OrderItem).where((OrderItem.order_id == order_id) & (OrderItem.product_id == product_id)))
    if order_item:
